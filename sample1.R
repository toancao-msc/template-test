...
#****************************************************
#divide the element of the row with the sum of the row
M2<-matrix(0,ncol=20,nrow=20)
for (i in 1:20){
  
  for (j in 1:20){
    M2[i,j]=M[i,j]/sum(M[i,])
  }
}
...
# find the invariant matrix,
r<-matrix(0,nrow=1,ncol=20)
for (i in 1:20) {
  r[1,i]=sum(M[i,])/sum(M)
}
...
Nsim=50000  # number of simulation
# 
# f<-rep(0,Nsim) # data storage
# for(r in 1:Nsim){
# d<-sample(a)
# d[21]<-d[1]
# e=0
